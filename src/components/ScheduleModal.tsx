...
const PEAK_TIMES = [
  { label: 'Today, 6:30 PM', sub: '🔥 Peak Engagement', date: 'Today', time: '6:30 PM (Peak)' },
  { label: 'Tomorrow, 12:15 PM', sub: '⚡ Lunch Break Surge', date: 'Tomorrow', time: '12:15 PM (Lunch)' },
  { label: 'Upcoming Weekend', sub: '🚀 Viral Velocity', date: 'Upcoming', time: '8:00 PM (Weekend)' },
];
...
export function ScheduleModal({
  ...
  // Sync state when meme changes
  React.useEffect(() => {
    if (meme) {
      setEditedHook(meme.hook);
      setEditedCaption(meme.caption || `Discover ${business.companyName}: ${business.keyBenefits}. Link in bio! 🚀`);
  ...
  const handleConfirm = (mode: 'calendar' | 'instagram') => {
    if (freeLimitReached) {
      onOpenPaywall();
      return;
    }
    ...
    const newPost: ScheduledPost = {
      ...
      viewsForecast: `${Math.floor(85 + Math.random() * 95)}k`,
      channel: 'instagram',
    };
      ...
      <div className="relative w-full max-w-md bg-neutral-900 border border-neutral-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-800 bg-neutral-900/90">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-bold text-white leading-tight">Schedule Post</h2>
              <p className="text-[11px] text-neutral-400">Targeting Peak Instagram Reel Audience</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 text-left">
          ...
          <div>
            ...
            >
              <div className="flex items-center gap-2.5">
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${postToInstagram ? 'bg-gradient-to-tr from-amber-500 via-rose-500 to-purple-600 text-white shadow-sm' : 'bg-neutral-800 text-neutral-400'}`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-semibold text-neutral-300">
                Recommended Peak Posting Slot
              </label>
              <span className="text-[10px] text-emerald-400 font-medium">Algorithmic Peak</span>
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-neutral-800 border border-neutral-700 rounded-xl px-3 py-2 text-xs text-white placeholder-neutral-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 resize-none"
            />
            <div className="flex flex-wrap gap-1 mt-1.5">
              {meme.hashtags.map((tag, i) => (
                <span key={i} className="text-[10px] font-mono bg-neutral-800 text-neutral-400 px-2 py-0.5 rounded-md border border-neutral-750">
                  {tag}
                </span>
              ))}
            </div>
          ...
          {freeLimitReached ? (
            <div
              ...
              className="p-3 bg-gradient-to-r from-amber-500/20 to-amber-600/10 border border-amber-500/30 rounded-2xl flex items-center justify-between cursor-pointer hover:border-amber-500/50 transition-all"
            >
              <div className="flex items-center gap-2">
                <Crown className="w-5 h-5 text-amber-400" />
                <div>
                  <p className="text-xs font-bold text-amber-300">Free Tier Limit Reached (3/3)</p>
                  <p className="text-[10px] text-amber-400/80">Upgrade to Pro for unlimited scheduled reels</p>
                </div>
              </div>
            ...
            <div className="p-2.5 bg-neutral-800/40 rounded-xl border border-neutral-800 flex items-center justify-between">
              <span className="text-[11px] text-neutral-400">
                Free Tier: <strong className="text-white">{currentScheduledCount} of 3</strong> scheduled posts used
              </span>
              <button
                type="button"
                onClick={onOpenPaywall}
